...
static cy_ble_gatt_db_attr_handle_t cy_ble_esscReqHandle[CY_BLE_GATTC_COUNT];

/* Read Long Descriptors variables */
static uint8_t * cy_ble_esscRdLongBuffPtr[CY_BLE_GATTC_COUNT] = { NULL };
static uint32_t cy_ble_esscRdLongBuffLen[CY_BLE_GATTC_COUNT] = { 0u };
static uint8_t cy_ble_esscCurrLen[CY_BLE_GATTC_COUNT] = { 0u };

uint8_t activeCharIndex[CY_BLE_GATTC_COUNT] = { 0u };
uint8_t activeCharInstance[CY_BLE_GATTC_COUNT] = { 0u };
uint8_t prevCharInstIndex[CY_BLE_GATTC_COUNT];
...
***************************************************************************//**
*
*  Registers a callback function for Environmental Sensing Service specific 
...
*     for Environmental Sensing Service is:\n
...
*  * \ref CY_BLE_EVT_ESSS_INDICATION_CONFIRMATION - In case if the indication is
...
static void Cy_BLE_ESSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_ESS_ApplCallback != NULL))
    {
...
static void Cy_BLE_ESSC_ExecuteWriteResponseEventHandler(const cy_stc_ble_gattc_exec_write_rsp_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_ESS_ApplCallback != NULL) && (cy_ble_esscReqHandle[discIdx] !=
                                                                                      CY_BLE_GATT_INVALID_ATTR_HANDLE_VALUE))
    {
        for(locCharIndex = CY_BLE_ESS_DESCRIPTOR_VALUE_CHANGED; (locCharIndex < CY_BLE_ESS_CHAR_COUNT) &&
            (locReqHandle == 0u); locCharIndex++)
...
*    successfully written on the peer device, the details (char index, etc.) are
                                                          ...
                                                          uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_ESS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
*    successfully read on the peer device, the details (char index , value, etc.) are
*    provided with event parameter structure of type \ref cy_stc_ble_ess_char_value_t.
*  .
*  Otherwise (if the ESS service-specific callback is not registered):
...
*    descr index etc.) are provided with event parameter structure
*    of type \ref cy_stc_ble_ess_descr_value_t.
*  .
*  Otherwise (if the ESS service-specific callback is not registered):
*  * \ref CY_BLE_EVT_GATTC_WRITE_RSP - In case if the requested attribute is
