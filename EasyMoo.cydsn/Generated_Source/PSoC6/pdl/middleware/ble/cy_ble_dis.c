...
static cy_ble_gatt_db_attr_handle_t cy_ble_discReqHandle[CY_BLE_GATTC_COUNT];
                                                          ...
                                                          cy_en_ble_dis_char_index_t charIndex)
{
    ...
    if((charIndex >= CY_BLE_DIS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
    else
...
static void Cy_BLE_DISC_ReadResponseEventHandler(cy_stc_ble_gattc_read_rsp_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_DIS_ApplCallback != NULL) &&
