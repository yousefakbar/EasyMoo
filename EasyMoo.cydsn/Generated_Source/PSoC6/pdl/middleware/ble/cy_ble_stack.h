...
#define CY_BLE_STACK_PATCH                                               (3u)

/** Firmware build number. */
#define CY_BLE_STACK_BUILD                                               (935u)
...
typedef enum
{
     ...
     *  the 'Advanced' tab in th BLE Component GUI and select the 'Show Expression Vew' option.
     */
    ...
    */
     ...
     *  wait for atleast 10ms, before calling the Cy_BLE_Start() API, after receiveing this event.
    ...
    /** This event is triggered when an advertisment packet is sent over the air. This event will
    ...
    /** This event indicates that the 'Prepare Write' Request is receicved from GATT Client device. The event parameter is a
...
*   | CY_BLE_EVT_HARDWARE_ERROR          | Generated by BLE Stack if initialization fails in Single Core Configuration      |
*   | CY_BLE_EVT_STACK_SHUTDOWN_COMPLETE | Generated by BLE Stack if initialization fails in Dual Core Configuration    |
*
...
*   for Single Core mode operation. Otherwise, the BLE Stack will throw an error.
*
...
***************************************************************************//**
*
* This function is used by BLE Component to request the BLE Sub-System (BLESS) to enter DeepSleep mode. This function should not be 
...
* Based on the current state of the radio and the link layer timing, BLESS may fail to enter DeepSleep mode. 
* The return value of this function should be used to check if the DeepSleep entry request was successful or not. 
...
*  2. DeepSleep (low power Mode) - All the high frequency blocks in BLESS and the BLE ECO are shutdown and the BLE link layer timing is 
...
* for an entry indication to BLESS DeepSleep Mode. No event is generated on calling this function.
*
