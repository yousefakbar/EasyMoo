...
static cy_ble_gatt_db_attr_handle_t cy_ble_ipscReqHandle[CY_BLE_GATTC_COUNT];

/* Read Long Descriptors variables */
static uint8_t *cy_ble_ipscRdLongBuffPtr[CY_BLE_GATTC_COUNT] = { NULL };
static uint16_t cy_ble_ipscRdLongBuffLen[CY_BLE_GATTC_COUNT] = { 0u };
static uint16_t cy_ble_ipscCurrLen[CY_BLE_GATTC_COUNT] = { 0u };
                                                                         ...
                                                                         uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_IPS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
                                                              ...
                                                              uint8_t *attrValue)
{
    ...
    else if((attrSize == 0u) || (attrValue == NULL) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
    else if(cy_ble_ipsc[discIdx].charInfo[charIndex].valueHandle == CY_BLE_GATT_INVALID_ATTR_HANDLE_VALUE)
...
static void Cy_BLE_IPSC_DiscoverCharacteristicsEventHandler(cy_stc_ble_disc_char_info_t *discCharInfo)
{
    /* Indoor Positioning Service characteristics UUIDs */
...
static void Cy_BLE_IPSC_ReadResponseEventHandler(cy_stc_ble_gattc_read_rsp_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_IPS_ApplCallback != NULL) &&
...
***************************************************************************//**
*
*  Handles a Execute Write Response event for the Indoor Positioning Service.
*
