    ...
    switch(event)
    {
        ...
        case CY_BLE_EVT_GAP_DEVICE_CONNECTED:
            {           
                ...
                if(tEvParam->status == CY_BLE_HCI_SUCCESS)
                {
                    ...
                    #if (CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_BROADCASTER)
                        if(tEvParam->role == CY_BLE_GAP_LL_ROLE_SLAVE)
                        {
                            Cy_BLE_SetAdvertisementState(CY_BLE_ADV_STATE_STOPPED);
                        }
                    #endif /* (CY_BLE_GAP_ROLE_PERIPHERAL || CY_BLE_GAP_ROLE_BROADCASTER) */
        ...
        case CY_BLE_EVT_GAPC_SCAN_START_STOP:
            ...
            if((cy_ble_eventHandlerFlag & CY_BLE_CALLBACK) != 0u)
            {
                ...
                else /* The following event indicates that scanning has been stopped by BLE Stack */
                {
        ...
        {
            cy_en_ble_gatt_err_code_t gattErr; 
            if((((cy_stc_ble_gatts_write_cmd_req_param_t*)evParam)->handleValPair.attrHandle > CY_BLE_GATT_DB_INDEX_COUNT) ||
               (((cy_stc_ble_gatts_write_cmd_req_param_t*)evParam)->handleValPair.attrHandle == 0u))
            {
                ...
                {
                    .errInfo.opCode     = CY_BLE_GATT_WRITE_REQ,
                    .errInfo.attrHandle = ((cy_stc_ble_gatts_write_cmd_req_param_t*)evParam)->handleValPair.attrHandle,        
                    .connHandle         = ((cy_stc_ble_gatts_write_cmd_req_param_t*)evParam)->connHandle,
                    .errInfo.errorCode  = CY_BLE_GATT_ERR_INVALID_HANDLE
                };
