...
static cy_ble_gatt_db_attr_handle_t cy_ble_cpscReqHandle[CY_BLE_GATTC_COUNT];

static uint8_t cy_ble_cpscFlag[CY_BLE_GATTC_COUNT];
static cy_stc_ble_timer_info_t cy_ble_cpscCpTimeout[CY_BLE_GATTC_COUNT];
...
***************************************************************************//**
*
*  This function initializes Cycling Power Service.
*
*  \param config: Configuration structure for Cycling Power Service.
*
*  \return
...
cy_en_ble_api_result_t Cy_BLE_CPS_Init(cy_stc_ble_cps_config_t *config)
{
    ...
    else
    {
        ...
        /* Registers Event Handler for the CPS Service */
...
***************************************************************************//**
*
*  Registers a callback function for Cycling Power Service service specific 
...
*    for Cycling Power Service is:<br>
                                                          ...
                                                          uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_CPS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
*     descr index, value, etc.) are provided with event parameter structure
*     of type \ref cy_stc_ble_cps_descr_value_t.
*   .
*   Otherwise (if an CPS service-specific callback is not registered):
...
static void Cy_BLE_CPSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_CPS_ApplCallback != NULL))
    {
