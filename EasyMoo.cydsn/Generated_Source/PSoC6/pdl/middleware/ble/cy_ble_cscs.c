...
static cy_ble_gatt_db_attr_handle_t cy_ble_cscscReqHandle[CY_BLE_GATTC_COUNT];
...
static void Cy_BLE_CSCSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_CSCS_ApplCallback != NULL))
    {
...
*    successfully written on the peer device, the details (char index, descr index etc.) are
...
*    successfully read on the peer device, the details (char index, descr index, value, etc.)
