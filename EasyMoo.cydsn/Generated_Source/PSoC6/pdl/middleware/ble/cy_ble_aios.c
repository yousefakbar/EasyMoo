...
static cy_ble_gatt_db_attr_handle_t cy_ble_aioscReqHandle[CY_BLE_GATTC_COUNT];
uint8_t cy_ble_aioscActiveCharIndex[CY_BLE_GATTC_COUNT] = { 0u };
uint8_t cy_ble_aioscActiveCharInstance[CY_BLE_GATTC_COUNT] = { 0u };
uint8_t cy_ble_aioscPrevCharInstIndex[CY_BLE_GATTC_COUNT] = { 0u };
...
*  \param config: Configuration structure for the Automation IO Service.
*
*  \return
...
static void Cy_BLE_AIOSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_AIOS_ApplCallback != NULL))
    {
                                                           ...
                                                           uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_AIOS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
