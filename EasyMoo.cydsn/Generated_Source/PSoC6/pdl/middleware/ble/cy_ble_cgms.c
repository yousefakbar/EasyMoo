...
static cy_ble_gatt_db_attr_handle_t cy_ble_cgmscReqHandle[CY_BLE_GATTC_COUNT];
...
***************************************************************************//**
*
*  This function initializes the Continuous Glucose Monitoring Service.
*
...
cy_en_ble_api_result_t Cy_BLE_CGMS_Init(cy_stc_ble_cgms_config_t *config)
{
    ...
    else
    {
        ...
        /* Registers Event Handler for the CGMS Service */
                                                           ...
                                                           uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_CGMS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
static void Cy_BLE_CGMSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_CGMS_ApplCallback != NULL))
    {
...
static void Cy_BLE_CGMSC_WriteResponseEventHandler(const cy_stc_ble_conn_handle_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_CGMS_ApplCallback != NULL) && (cy_ble_cgmscReqHandle[discIdx] !=
                                                                                       CY_BLE_GATT_INVALID_ATTR_HANDLE_VALUE))
    {
        cy_en_ble_cgms_char_index_t charIdx;
        uint8_t fFlag = 0u;
