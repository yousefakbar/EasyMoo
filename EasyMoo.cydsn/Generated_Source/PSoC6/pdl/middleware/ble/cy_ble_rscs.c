...
/* The internal storage for the last request handle to check response */
static cy_ble_gatt_db_attr_handle_t cy_ble_rscscReqHandle[CY_BLE_GATTC_COUNT];
...
static void Cy_BLE_RSCSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_RSCS_ApplCallback != NULL))
    {
                                                                ...
                                                                uint8_t *attrValue)
{
    ...
    {
        if((discIdx < CY_BLE_GATTC_COUNT) && ((charIndex == CY_BLE_RSCS_RSC_MEASUREMENT) ||
           (charIndex == CY_BLE_RSCS_SC_CONTROL_POINT)))
        {
            /* Fill all fields of write request structure ... */
