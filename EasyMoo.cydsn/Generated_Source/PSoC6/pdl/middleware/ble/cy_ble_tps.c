...
static cy_ble_gatt_db_attr_handle_t cy_ble_tpscReqHandle[CY_BLE_GATTC_COUNT];
...
static void Cy_BLE_TPSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_TPS_ApplCallback != NULL))
    {
                                                          ...
                                                          cy_en_ble_tps_char_index_t charIndex)
{
    ...
    if((charIndex == CY_BLE_TPS_TX_POWER_LEVEL) && (discIdx < CY_BLE_GATTC_COUNT))
    {
