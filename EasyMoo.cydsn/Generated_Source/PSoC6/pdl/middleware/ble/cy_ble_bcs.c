...
static cy_ble_gatt_db_attr_handle_t cy_ble_bcscReqHandle[CY_BLE_GATTC_COUNT];
...
*  * \ref CY_BLE_EVT_BCSS_INDICATION_CONFIRMED - If the indication is
...
static void Cy_BLE_BCSC_IndicationEventHandler(cy_stc_ble_gattc_handle_value_ind_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_BCS_ApplCallback != NULL))
    {
                                                          ...
                                                          cy_en_ble_bcs_char_index_t charIndex)
{
    ...
    else if((charIndex >= CY_BLE_BCS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
*   .
*   Otherwise (if an BCS service-specific callback is not registered):
