...
static cy_ble_gatt_db_attr_handle_t cy_ble_udscReqHandle[CY_BLE_GATTC_COUNT];

/* Read Long Descriptors variables */
static uint8_t * cy_ble_udscRdLongBuffPtr[CY_BLE_GATTC_COUNT] = { NULL };
static uint16_t cy_ble_udscRdLongBuffLen[CY_BLE_GATTC_COUNT] = { 0u };
static uint8_t cy_ble_udscCurrLen[CY_BLE_GATTC_COUNT] = { 0u };
                                                          ...
                                                          uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_UDS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
static void Cy_BLE_UDSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_UDS_ApplCallback != NULL) &&
...
static void Cy_BLE_UDSC_ReadResponseEventHandler(cy_stc_ble_gattc_read_rsp_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (cy_ble_udscReqHandle[discIdx] != CY_BLE_GATT_INVALID_ATTR_HANDLE_VALUE))
    {
        if(Cy_BLE_UDS_ApplCallback != NULL)
        {
