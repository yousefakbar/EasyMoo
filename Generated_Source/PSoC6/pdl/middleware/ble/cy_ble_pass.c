...
static cy_ble_gatt_db_attr_handle_t cy_ble_passcReqHandle[CY_BLE_GATTC_COUNT];
                                                           ...
                                                           uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_PASS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
static void Cy_BLE_PASSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_PASS_ApplCallback != NULL))
    {
        cy_stc_ble_pass_char_value_t locCharValue =
        {
            .connHandle = eventParam->connHandle,
