...
static cy_ble_gatt_db_attr_handle_t cy_ble_llscReqHandle[CY_BLE_GATTC_COUNT];
...
static void Cy_BLE_LLSC_ReadResponseEventHandler(cy_stc_ble_gattc_read_rsp_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_LLS_ApplCallback != NULL) &&
                                                          ...
                                                          uint8_t *attrValue)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (attrValue != NULL) && (charIndex == CY_BLE_LLS_ALERT_LEVEL) &&
                                                          ...
                                                          cy_en_ble_lls_char_index_t charIndex)
{
    ...
    else if((charIndex == CY_BLE_LLS_ALERT_LEVEL) && (discIdx < CY_BLE_GATTC_COUNT))
    {
        /* Send request to write Alert Level characteristic value */
