              ...
              (flashReturn == CY_FLASH_DRV_SUCCESS) && (srcIndex < length))
        {
            ...
            for(dstIndex = 0u; dstIndex < CY_FLASH_SIZEOF_ROW; dstIndex++)
            {
                if((byteOffset >= eeOffset) && (srcIndex < length))
                {
                    ...
                    if((rowsNotEqual == false) && (CY_GET_REG8(CY_FLASH_BASE + byteOffset) != buffer[srcIndex]))
                    {
                        rowsNotEqual = true;
                    }
                    srcIndex++;
...
uint32_t Cy_BLE_HAL_GetIcPackageType(void)
{
    ...
    uint8_t pType = CY_GPIO_PACKAGE_TYPE;

#if defined(CY_GPIO_PIN_COUNT) 
    uint8_t pinCount = CY_GPIO_PIN_COUNT;
#else
    uint8_t pinCount = 0;
    ...
    if( ((pType == CY_GPIO_PACKAGE_BGA) && (pinCount == 0u)) ||
        ((pType == CY_GPIO_PACKAGE_BGA) && (pinCount == PACKAGE_116_PINS)) )
    {
        ret = CY_BLE_IC_PACKAGE_TYPE_BGA_116;
    }
    else if( ((pType == CY_GPIO_PACKAGE_CSP) && (pinCount == 0u)) ||
             ((pType == CY_GPIO_PACKAGE_CSP) && (pinCount == PACKAGE_104_PINS)) )
    {
        ret = CY_BLE_IC_PACKAGE_TYPE_CSP_104;
    }
    else if((pType == CY_GPIO_PACKAGE_BGA) && (pinCount == PACKAGE_124_PINS))
    {
        ret = CY_BLE_IC_PACKAGE_TYPE_BGA_124;
    }
    else if((pType == CY_GPIO_PACKAGE_QFN) && (pinCount == PACKAGE_56_PINS))
    {
...
****************************************************************************//**
*
* This function returns absolute value temperature.
*
