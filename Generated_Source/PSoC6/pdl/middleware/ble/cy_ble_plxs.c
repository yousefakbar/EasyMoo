...
static cy_ble_gatt_db_attr_handle_t cy_ble_plxscReqHandle[CY_BLE_GATTC_COUNT];

/* RACP Procedure Timeout TIMER setting */
static cy_stc_ble_timer_info_t cy_ble_plxscRacpTimeout[CY_BLE_GATTC_COUNT];

/* PLXS Client flags */
uint32_t cy_ble_plxscFlag[CY_BLE_GATTC_COUNT];
                                                           ...
                                                           uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_PLXS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
static void Cy_BLE_PLXSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_PLXS_ApplCallback != NULL))
    {
        cy_stc_ble_plxs_char_value_t locCharValue = { .connHandle = eventParam->connHandle };
