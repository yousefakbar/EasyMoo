...
cy_stc_ble_hidsc_t cy_ble_hidsc[CY_BLE_GATTC_COUNT][CY_BLE_HIDSC_SERVICE_COUNT];

/* The internal storage for the last request handle to check response */
static cy_ble_gatt_db_attr_handle_t cy_ble_hidscReqHandle[CY_BLE_GATTC_COUNT];

uint8_t cy_ble_hidscServiceCount[CY_BLE_GATTC_COUNT];

/* To discovery descriptors for multiple HID service instances */
uint8_t cy_ble_hidscDisServiceIndex[CY_BLE_GATTC_COUNT];
...
static void Cy_BLE_HIDSC_DiscoverCharacteristicsEventHandler(cy_stc_ble_disc_char_info_t *discCharInfo)
{
    ...
    static uint32_t discoveryLastServ[CY_BLE_GATTC_COUNT] = { 0u };
...
static void Cy_BLE_HIDSC_ReadResponseEventHandler(cy_stc_ble_gattc_read_rsp_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_HIDS_ApplCallback != NULL) &&
