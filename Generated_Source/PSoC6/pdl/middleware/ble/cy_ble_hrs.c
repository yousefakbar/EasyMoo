...
static cy_ble_gatt_db_attr_handle_t cy_ble_hrscReqHandle[CY_BLE_GATTC_COUNT];
...
***************************************************************************//**
*
*  Registers a callback function for Heart Rate Service specific attribute operations.
*  Service specific write requests from peer device will not be handled with
*  unregistered callback function.
*
*  \param callbackFunc: An application layer event callback function to receive
*    events from the BLE Middleware. The definition of cy_ble_callback_t for 
*    Heart Rate Service is: \n
                                                          ...
                                                          uint8_t * attrValue)
{
    ...
    else if((charIndex >= CY_BLE_HRS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT) ||
...
static void Cy_BLE_HRSC_ReadResponseEventHandler(cy_stc_ble_gattc_read_rsp_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_HRS_ApplCallback != NULL) &&
