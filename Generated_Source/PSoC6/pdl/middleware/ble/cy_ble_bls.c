...
static cy_ble_gatt_db_attr_handle_t cy_ble_blscReqHandle[CY_BLE_GATTC_COUNT];
                                                          ...
                                                          cy_en_ble_bls_char_index_t charIndex)
{
    ...
    else if((charIndex >= CY_BLE_BLS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
...
static void Cy_BLE_BLSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_BLS_ApplCallback != NULL) &&
