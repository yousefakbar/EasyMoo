...
static cy_ble_gatt_db_attr_handle_t cy_ble_wsscReqHandle[CY_BLE_GATTC_COUNT];
...
static void Cy_BLE_WSSC_IndicationEventHandler(cy_stc_ble_gattc_handle_value_ind_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_WSS_ApplCallback != NULL))
    {
                                                          ...
                                                          cy_en_ble_wss_char_index_t charIndex)
{
    ...
    else if((charIndex >= CY_BLE_WSS_CHAR_COUNT) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
