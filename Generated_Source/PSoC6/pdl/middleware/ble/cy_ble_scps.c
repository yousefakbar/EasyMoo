...
static cy_ble_gatt_db_attr_handle_t cy_ble_scpscReqHandle[CY_BLE_GATTC_COUNT];
                                                           ...
                                                           uint8_t * attrValue)
{
    ...
    if((charIndex != CY_BLE_SCPS_SCAN_INT_WIN) || (discIdx >= CY_BLE_GATTC_COUNT))
    {
        apiResult = CY_BLE_ERROR_INVALID_PARAMETER;
    }
    else
...
static void Cy_BLE_SCPSC_NotificationEventHandler(cy_stc_ble_gattc_handle_value_ntf_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_SCPS_ApplCallback != NULL))
    {
