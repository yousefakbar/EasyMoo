...
static cy_ble_gatt_db_attr_handle_t cy_ble_ndcscReqHandle[CY_BLE_GATTC_COUNT];
...
cy_en_ble_api_result_t Cy_BLE_NDCS_Init(cy_stc_ble_ndcs_config_t *config)
{
    ...
    else
    {
        ...
        /* Registers Event Handler for the NDCS Service */
...
static void Cy_BLE_NDCSC_ReadResponseEventHandler(cy_stc_ble_gattc_read_rsp_param_t *eventParam)
{
    ...
    if((discIdx < CY_BLE_GATTC_COUNT) && (Cy_BLE_NDCS_ApplCallback != NULL) &&
                                                           ...
                                                           cy_en_ble_ndcs_char_index_t charIndex)
{
    ...
    else if((charIndex == CY_BLE_NDCS_TIME_WITH_DST) && (discIdx < CY_BLE_GATTC_COUNT))
    {
